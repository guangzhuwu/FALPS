...
void VodApp::requestScheduling_RF_Rnd_2()
{
	...
	for (piece_id_t i=startPiece;candidatePieces.size()<num&&i<endpiece;++i)
	{
		...
		if (!m_playingBuffer.exist(i))
		{
			...
			if (pieceInfo.holders.size()>0)
			{
				...
				pieceInfo.priority=1.0/(pieceInfo.holders.size()+priorityBecauseWait)*Rand01();
				allPriority+=pieceInfo.priority;
				candidatePieces.push_back(pieceInfo);
				if (m_player.state()==Player::NOT_START)
				{
					//pieceInfo.holders.push_back(&m_supperNeightbor);
			...
			else 
			{
				...
				if (pieceInfo.holders.size()>0)
				{
					pieceInfo.priority=1.0/(m_intreastingNB.size()+priorityBecauseWait)*Rand(0,0.05);
					allPriority+=pieceInfo.priority;
					//pieceInfo.priority=Rand(0,1);
					candidatePieces.push_back(pieceInfo);
				}
	...
	//for (vector<piece_info>::iterator itr(candidatePieces.begin());itr!=candidatePieces.end();itr++)
	//{
	//	itr->priority/=allPriority;
