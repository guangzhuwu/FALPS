...
void VodApp::requestScheduling_RF()
{
	...
	for (piece_id_t i=startPiece;candidatePieces.size()<num&&i<endpiece;++i)
	{
		...
		if (!m_playingBuffer.exist(i))
		{
			...
			if (pieceInfo.holders.size()>0)
			{
				...
				//pieceInfo.priority=1.0/(pieceInfo.holders.size()+priorityBecauseWait);
				pieceInfo.priority=1.0/(pieceInfo.holders.size()+priorityBecauseWait)+Rand(0,0.001);
				//pieceInfo.priority=1.0/ability+Rand(0,0.001);
			...
			else 
			{
				...
				pieceInfo.holders.push_back(&m_supperNeightbor);
				ability+=m_supperNeightbor.ability;
				//else if(m_maxDownloadRate)//否则以一定的概率将服务器加入
				//	pieceInfo.holders.push_back(&m_supperNeightbor);
				if (pieceInfo.holders.size()>0)
				{
					//pieceInfo.priority=1.0/(100+priorityBecauseWait);
					pieceInfo.priority=1.0/(100+priorityBecauseWait)+Rand(0,0.001);
					//pieceInfo.priority=1.0/ability+Rand(0,0.001);
					//pieceInfo.priority=Rand(0,1);
					candidatePieces.push_back(pieceInfo);
				}
	...
	double pri=-1;
	set<piece_id_t> selectpiece;
	for(int k=0;k<4;k++)
	{
		set<piece_id_t> tmpselectpiece;
		requestCnt=0;
		double tmpPri=0;
		for (size_t i=0;i<candidatePieces.size();++i)
		{
			...
			for (size_t j=0;j<4*holders.size();j++)
			{
				...
				if ((holderInfo->ability+0.5)>(needAbility))
				{
					...
					++requestCnt;
					tmpselectpiece.insert(candidatePieces[i].pieceID);
					break;
				}
			}
	...
	int cnt=min((double)maxRequestCntPerCycle,1.0/PLAY_TIME_PER_PIECE);
	if(requestCnt<cnt)
	{
		for (size_t i=0;i<candidatePieces.size();++i)
		{
			if(selectpiece.find(candidatePieces[i].pieceID)==selectpiece.end())
			{
				...
				if (candidatePieces[i].isUrgent)
					...
					holderInfo->normalRequestsCurrCycle.push_back(candidatePieces[i].pieceID);
				++requestCnt;
				selectpiece.insert(candidatePieces[i].pieceID);
			}
			if(requestCnt>=cnt)
				break;
		}
